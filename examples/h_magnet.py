...
def run_h_magnet(verify=False, dirichlet='soft', gauge=True, legacy=False):
    ...
    print(f'b_max = {max(np.linalg.norm(b.elementValues.values, axis=1)):.4f}')
    if field.isGauged():
        assert abs(max(np.linalg.norm(b.elementValues.values, axis=1)) - 3.1892) < 2e-3
    else:
        assert abs(max(np.linalg.norm(b.elementValues.values, axis=1)) - 3.1892) < 2e-3
