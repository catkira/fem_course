...
class DofFieldData:
    def __init__(self, field):
        ...
        if m.getMesh()['problemDimension'] == 3:
            # assume dirichlet BCs are always on problemDimension -1
            self.dirichletMask = np.repeat(False, m.numberOfTriangles())
        self.field = field
        self.startIndex = 0
...
def updateFieldRegions(field):
    ...
    for region in field.regions:
        for dim in range(m.getMesh()['problemDimension']):
            if np.any(m.getMesh()['physical'][dim]) is not None and region in m.getMesh()['physical'][dim]:
                elements = m.getElements(field.isEdgeField(), dim)
                if elements is None:
                    continue
                if field.isEdgeField():
                    dofManagerData.fields[field.id].freeEdgesMask[elements[region == m.getMesh()['physical'][dim]]] \
                        = True
                else:
                    dofManagerData.fields[field.id].freeNodesMask[elements[region == m.getMesh()['physical'][dim]]] \
                        = True
