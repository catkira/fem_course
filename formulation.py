...
from utils import bcolors, gaussData
import dofManager as dm
from mesh import getMesh, getSigns, transformationJacobians, transformationJacobian, numberOfBoundaryEdges, numberOfVertices
...
# pip install
# https://github.com/pyvista/pyvista-wheels/raw/main/vtk-9.1.0.dev0-cp310-cp310-win_amd64.whl
# pip install
...
def globalCoordinate(g, _, xi):
    b, x1 = transformationJacobian(g)

    return x1 + b @ xi


def localCoordinate(g, _, x):
    b, x1 = transformationJacobian(g)
...
def matrix_gradDofV_tfA(fieldV, fieldA, sigmas, region): # pylint: disable=C0103
...
def matrix_DofA_gradTfV(fieldA, fieldV, sigmas, region):  # pylint: disable=C0103
    elementsV = fieldV.getElements(region=region)
    elementsA = fieldA.getElements(region=region)
    sigmas = sigmas.getValues(region)
    elementDim = region.regionDimension
    ...
    if not legacy:
        ...
        if len(sigmas.shape) == 1:
            ...
            for elementIndex in range(numElements):  # TODO: vectorize this
                gammas[elementIndex] = (
                    sigmas[elementIndex]
                    @ invJacs[elementIndex]
                    @ np.swapaxes(invJacs, 1, 2)[elementIndex]
                    * detJacsDuplicated[elementIndex]
                )
        if vectorized:
            data = np.einsum("ijk,jklm", gammas, B).ravel(order="C")
        else:
            for elementIndex in range(numElements):
                indexRange = np.arange(
                    start=elementIndex * elementMatrixSize,
                    stop=elementIndex * elementMatrixSize + elementMatrixSize,
                )
        ...
        if getMesh()["problemDimension"] == 3:
            ...
            detJacsDuplicated = np.repeat(detJacs, elementDim**2).reshape((numElements, elementDim, elementDim))
            gammas = sigmasDuplicated * invJacs @ np.swapaxes(invJacs, 1, 2) * detJacsDuplicated
            for elementIndex, _ in enumerate(elements):
                indexRange = np.arange(
                    start=elementIndex * elementMatrixSize,
                    stop=elementIndex * elementMatrixSize + elementMatrixSize,
                )
                data[indexRange] = np.einsum(
                    "jk,jk...", gammas[elementIndex], B
                ).ravel()  # this is generic and faster than explicit summation like below

        if getMesh()["problemDimension"] == 2:
            # area of ref triangle is 0.5, integrands are constant within integral
            B11 = 0.5 * np.matrix(Grads[:, 0]).T * np.matrix(Grads[:, 0])
            B12 = 0.5 * np.matrix(Grads[:, 0]).T * np.matrix(Grads[:, 1])
            B21 = 0.5 * np.matrix(Grads[:, 1]).T * np.matrix(Grads[:, 0])
            B22 = 0.5 * np.matrix(Grads[:, 1]).T * np.matrix(Grads[:, 1])
            # K_Ts = np.zeros([m,3,3])
            for elementIndex, _ in enumerate(elements):
                ...
                if len(sigmas.shape) == 1:
                    ...
                    sigmaDash = invJac @ sigmas[elementIndex] @ invJac.T * detJac
                    gamma11 = sigmaDash[0, 0]
                    gamma12 = sigmaDash[1, 0]
                    gamma21 = sigmaDash[0, 1]
                    gamma22 = sigmaDash[1, 1]
                indexRange = np.arange(
                    start=elementIndex * elementMatrixSize,
                    stop=elementIndex * elementMatrixSize + elementMatrixSize,
                )
                data[indexRange] = (gamma11 * B11 + gamma12 * B12 + gamma21 * B21 + gamma22 * B22).ravel()
...
def fluxRhsCurl(field, br: Parameter, region=None):
    if region is None:
        elements = getMesh()["pt"]
        elementDim = getMesh()["problemDimension"]
        elements = field.getElements(dim=elementDim)
...
def loadRhs(field, j: Parameter, region=None):
    if region is None:
        elementDim = getMesh()["problemDimension"]
        elements = field.getElements(dim=elementDim)
        jacs = transformationJacobians([], elementDim)
    ...
    for i, gp in enumerate(gps):
        for m in range(nBasis):
            if field.isEdgeField():
                data[:, m] += gfs[i] * np.einsum(
                    "i,ij,i,j->i",
                    detJacs,
                    jTransformed,
                    signs[:, m],
                    field.shapeFunctionValues(gp, elementDim)[m, :],
                )
            else:
                data[:, m] += gfs[i] * np.einsum(
                    "i,ij,j->i",
...
def fluxRhs(field, br: Parameter, region=None, vectorized=True):
    ...
    if vectorized:
        ...
        # temp2 = area* np.repeat(detJacs,nCoords*nBasis).reshape((len(detJacs),nCoords,nBasis))
        #         * np.einsum('ikj,lk', invJacs, Grads)
...
# integral rho * u * tf(u)
def massMatrixCurl(field1, field2, rhos: Parameter, region=None, elementDim=2, verify=False):
    if isinstance(region, (list, np.ndarray)):
    ...
    cols = np.repeat(elements1, nBasis).astype(np.int64).ravel()
    data2 = np.zeros((len(elements1), nBasis, nBasis))
    if elementDim in (2, 3):  # should work for any dimension
        detJacs = np.abs(np.linalg.det(jacs))
        invJacs = np.linalg.inv(jacs)
        calcMethod = 2
        if calcMethod == 1 or verify:  # this method is garbage, but it works
            ...
            for i, gp in enumerate(gps):
                for m in range(nBasis):
                    for k in range(nBasis):
                        Mm[m, k] = Mm[m, k] + gfs[i] * np.einsum(
                            "i,j->ij",
                            field1.shapeFunctionValues(gp, elementDim)[:, m],
                            field2.shapeFunctionValues(gp, elementDim)[:, k],
                        )
...
# integral rho * u * tf(u)
def massMatrix(field, rhos: Parameter, region=None, elementDim=2, vectorized=True):
    # n = dm.countAllFreeDofs()
    if isinstance(region, (list, np.ndarray)):
        ...
        sys.exit()
    if elementDim == 1:
        Mm = 1 / 6 * np.array([[2, 1], [1, 2]])
        nBasis = 2
    elif elementDim == 2:
        nBasis = 3
        if False: # pylint: disable=W0125
            Mm = 1 / 24 * np.array([[2, 1, 1], [1, 2, 1], [1, 1, 2]])
        else:
            # precalculate mesh independant parts of the integral
            ...
            for i, gp in enumerate(gps):
                for j in range(nBasis):
                    for k in range(nBasis):
                        Mm[j, k] += (
                            gfs[i]
                            * field.shapeFunctionValues(gp, elementDim)[j]
...
def boundaryMassMatrix(field, alphas, region=None):
    # Grads = field.shapeFunctionGradients()
    Bb = 1 / 6 * np.array([[2, 1], [1, 2]])
    ...
    for elementIndex, element in enumerate(elements):
        ...
        idxRange = np.arange(start=elementIndex * 4, stop=elementIndex * 4 + 4)
        rows[idxRange] = np.tile(element[:], 2).astype(np.int64)
        cols[idxRange] = np.repeat(element[:], 2).astype(np.int64)
        data[idxRange] = (alphas[elementIndex] * detJac * Bb).ravel()
...
def solve(A, b, method="np"):
    ...
    if method == "sparse":
        ...
        n = len(b)
        Ap = PETSc.Mat().createAIJWithArrays(
            size=(n, n),
        ...
        print(
            f"Solving {n} dofs with: {PC.getFactorSolverType():s} {ksp.getType():s} and preconditioner " \
                "{ksp.getPC().getType():s}"
        )
        ...
        sys.exit()
    numDofs = len(u)
    assert numDofs == dm.countAllFreeDofs()
    dm.putSolutionIntoFields(u)
    stop = time.time()
